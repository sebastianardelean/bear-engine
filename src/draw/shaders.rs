...
impl Shader {
    pub fn new(vertex_shader_file:&str, fragment_shader_file:&str) -> io::Result<Shader> {
        ...
        let mut buf_reader_vertex:BufReader<File> = BufReader::new(File::open(vertex_shader_file)?);
        buf_reader_vertex.read_to_string(&mut contents_vertex).unwrap_or_else(|e| {
        ...
        let mut buf_reader_fragment:BufReader<File> = BufReader::new(File::open(fragment_shader_file)?);
        buf_reader_fragment.read_to_string(&mut contents_fragment).unwrap_or_else(|e| {
            error_log!("Failed to initialize vertex shader: {}", e);
            panic!("Failed!");
        });
    ...
    fn compile(&mut self) -> (GLuint,GLuint) {
        ...
        unsafe {
            ...
            if success == 0 {
                ...
                gl::GetShaderInfoLog(vertex, message_len, std::ptr::null_mut(), buffer.as_mut_ptr() as *mut GLchar);
                let log = String::from_utf8_lossy(&buffer);
                error_log!("Error {} when compiling shader {} ", log,self.vertex_shader);
            }
    ...
    fn link(&mut self,vs: GLuint, fs: GLuint)-> GLuint {
        ...
        unsafe {
            ...
            if success == 0 {
                ...
                let mut buffer = Vec::with_capacity(message_len as usize);
                buffer.set_len((message_len as usize) - 1);
                gl::GetProgramInfoLog(self.id, message_len, std::ptr::null_mut(), buffer.as_mut_ptr() as *mut GLchar);
                let log = String::from_utf8_lossy(&buffer);
